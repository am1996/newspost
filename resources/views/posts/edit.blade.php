...
<div class="container bg-transparent p-4 mt-5">
    <h1 class="text-center m-2">Edit Post</h1>
    <form method="post">
        @csrf
        <div>
            <label for="editor" class="text-gray-600 font-semibold">Title</label>
            <input value="{{$post->title}}" name="title" type="text" name="title" class="form-control" style="background-color:#fff;">
        </div>
        ...
        <div>
            <button type="submit" class="btn btn-outline-success form-control">Edit Post</button>
        </div>
    </form>
</div>
<script>
